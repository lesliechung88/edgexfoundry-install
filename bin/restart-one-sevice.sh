...
if [ ! -n "$1" ] ;then
    ...
    echo "./restart-one-service.sh mongo XXX服务名"
    echo "或者"
    echo "./restart-one-service.sh redis XXX服务名"
    exit
else
       if [[ "$1" = "mongo" ]]; then
           ...
           if [[ "$mongoRuning" != "" ]]; then
                if [ ! -n "$2" ] ;then
                    ...
                    restart_service  ../compose-files/docker-compose-mongo.yml $2
                fi
            else
           ...
           if [[ "$redisRuning" != "" ]]; then
                if [ ! -n "$2" ] ;then
                   ...
                   restart_service  ../compose-files/docker-compose-redis.yml $2
                fi
            else
